...
namespace Z0
{
    ...
    //    }
        ...
        // public Outcome RunProjectScript(ProjectId project, string srcid, string scope, ScriptId script, bool quiet, out ReadOnlySpan<CmdFlow> flows)
        // {
        //     var path = Ws.Projects().Script(project, scope, script, FileKind.Cmd);
        //     var result = Outcome.Success;
        ...
        // {
        //     var path = Ws.Projects().Script(project, script, FileKind.Cmd);
        //     var result = Outcome.Success;
        ...
        //     var paths = Ws.Projects().Scripts(project).Files(FS.Cmd).Where(x => x.Format().StartsWith(match));
        ...
        //     => ScriptRunner.RunCmd(CmdScript.cmdline(src.Format(PathSeparator.BS)), vars, ReceiveCmdStatusQuiet, ReceiveCmdError, out response);
        ...
        // public Outcome RunProjectScript(ProjectId project, string src, string scope, ScriptId script)
        //     => RunProjectScript(project, src, scope, script, out _);

        // public Outcome RunProjectScript(ProjectId project, string src, string scope, ScriptId script, bool quiet)
        //     => RunProjectScript(project, src, scope, script, quiet, out _);

        // public Outcome RunProjectScript(ProjectId project, string src, string scope, ScriptId script, out ReadOnlySpan<CmdFlow> flows)
