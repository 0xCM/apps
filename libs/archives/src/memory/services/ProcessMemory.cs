...
namespace Z0
{
    ...
    public class ProcessMemory : AppService<ProcessMemory>
    {
        ...
        public static MemoryAddress @base(Name procname)
        {
            ...
            var module = ProcessMemory.modules(Process.GetCurrentProcess()).Where(m => Path.GetFileNameWithoutExtension(m.ImagePath.Name) == match).First();
        ...
        {
            var images = ProcessMemory.locate(src);
            ...
            fill(src, ref state);
            return new ProcessImageMap(state, images, addresses.Sort(), ProcessMemory.modules(src));
        }
        ...
        {
            var records = ProcessMemory.partitions(src);
