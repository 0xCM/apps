...
{
    [ApiHost]
    public partial class SettingIndex : IIndex<Setting>, ILookup<string,Setting>
    {
        [MethodImpl(Inline)]
            ...
            where T : new()
        {
            ...
            return SettingIndex.single(src.ReadLines(), sep, out dst);
        }
            ...
            where T : new()
        {
            ...
            for(var i=0; i<count; i++)
            {
                ...
                if(fields.TryGetValue(setting.Name, out var field))
                {
                    ...
                    result = SettingIndex.parse(setting.Format(sep), type, out var s);
                    if(result.Fail)
                        break;
