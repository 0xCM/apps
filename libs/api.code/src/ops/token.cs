...
namespace Z0
{
    ...
    {
        public static ApiToken token(ICompositeDispenser symbols, in MethodEntryPoint entry, MemoryAddress target)
            => new ApiToken(
                symbols.Symbol(entry.Location, entry.Uri?.Format() ?? EmptyString),
                symbols.Symbol(target, entry.Sig.Format()));

        public static ApiToken token(ICompositeDispenser symbols, in MethodEntryPoint entry)
            => new ApiToken(
