...
namespace Z0
{
    ...
    partial class XedRules
    {
        ...
        public class RuleMacros
        {
            ...
            [MethodImpl(Inline), Op]
            public static MacroSpec otherwise()
                => M.otherwise;
            ...
            public static RuleSet expand(RuleSet src)
            {
                RuleMacros.expand(src.Tables);
                expand(src.Patterns);
                return src;
            }

            public static void expand(Index<RulePattern> src)
            {
            ...
            public static RulePattern expand(in RulePattern pattern)
            {
                var tokens = list<RuleToken>(32);
                var assign = span<FieldAssign>(12);
                for(var j=0; j<pattern.Tokens.Count;j++)
                {
                    ...
                    var count = RuleMacros.expand(token,assign);
                    if(count > 0)
                    {
            ...
            public static Index<RuleToken> expand(Index<RuleToken> src)
            {
            ...
            public static void expand(Index<InstDef> src)
            {
                for(var i=0; i<src.Count; i++)
                    expand(MacroLookup, src[i].PatternSpecs);
            }

            public static void expand(Index<RulePatternInfo> src)
            {
                expand(MacroLookup,src);
            }

            public static void expand(Index<InstPatternSpec> src)
            {
                expand(MacroLookup, src);
            }

            public static void expand(ConstLookup<RuleMacroKind,MacroSpec> lu, Index<InstPatternSpec> src)
            {
                for(var i=0; i<src.Count; i++)
                {
            ...
            public static string expand(ConstLookup<RuleMacroKind,MacroSpec> specs, string src)
            {
            ...
            public static Index<RuleTermTable> expand(Index<RuleTermTable> src)
            {
                var count = src.Count;
                for(var i=0; i<count; i++)
                    expand(MacroLookup, ref src[i]);
                return src;
            }

            public static void expand(ConstLookup<RuleMacroKind,MacroSpec> specs, Index<RulePatternInfo> src)
            {
                var count = src.Count;
                for(var i=0; i<count; i++)
                {
                    ref var pattern = ref src[i];
                    expand(MacroLookup, ref pattern);
                }
            }

            public static void expand(ConstLookup<RuleMacroKind,MacroSpec> specs, ref RuleTermTable src)
            {
                ref var expressions = ref src.Expressions;
                var count = expressions.Count;
                for(var i=0; i<count; i++)
                    expand(specs, ref expressions[i]);
            }

            public static void expand(ConstLookup<RuleMacroKind,MacroSpec> specs, ref RuleTermExpr src)
            {
                expand(specs, src.Premise);
                expand(specs, src.Consequent);
            }

            public static void expand(ConstLookup<RuleMacroKind,MacroSpec> specs, Index<RuleTerm> src)
            {
                var count = src.Count;
                for(var i=0; i<count; i++)
                    expand(specs, ref src[i]);
            }

            public static void expand(ConstLookup<RuleMacroKind,MacroSpec> specs, ref RulePatternInfo src)
            {
                src.Expression = expand(specs, src.Expression);
            }

            public static void expand(ConstLookup<RuleMacroKind,MacroSpec> specs, ref RuleTerm src)
            {
