...
namespace Z0.Asm
{
    ...
    partial class AsmModelGen
    {
        ...
        {
            dst.IndentLine(margin, string.Format(StatementFactoryDefaultPattern, mnemonic.Format(MnemonicCase.Captialized), MonicFactoryName(mnemonic)));
            dst.AppendLine();
            dst.IndentLine(margin, InlineOpAttributeSpec);
            dst.IndentLine(margin, string.Format(StatementFactoryArgPattern, mnemonic.Format(MnemonicCase.Captialized), MonicFactoryName(mnemonic)));
            dst.AppendLine();
        }
    }
}