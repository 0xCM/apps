    ...
    partial class StringTables
    {
            ...
            where K : unmanaged
        {
            ...
            for(var i=0; i<count; i++)
            {
                ...
                seek(strings, bw32(entry.Key)) = entry.Value;
            }
            ...
            for(var i=0u; i<count; i++)
            {
                ref readonly var entry = ref src[i];
                seek(cuts, i) = j;
                StringTables.copy(entry.Value, ref j, chars);
            }
        ...
        internal static ulong copy(ReadOnlySpan<char> src, ref uint i, Span<char> dst)
        {
            var i0 = i;
            var count = src.Length;
