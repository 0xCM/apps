...
namespace Z0.Asm
{
    ...
    public class AsmOcDatasets
    {
        ...
        AsmOcDatasets()
        {
            ...
            // var kinds = Symbols.index<AsmOcTokenKind>();
            // TokenKindSymbols = kinds;
            // TokenSet = OpCodeTokenSet.create();
            // var tokenExpr = dict<AsmOcToken,string>();
            // var exprToken = dict<string,AsmOcToken>();
            // var tokens = TokenSet.View;
            // var hexcount = hex.Count;
            // var tokencount = (uint)tokens.Length;
            // Tokens = alloc<AsmOcToken>(tokencount + hexcount);
            // var j=0;
            ...
            // TokensByKind = Tokens.GroupBy(t => t.Kind).Select(x => (x.Key, (Index<AsmOcToken>)x.Array())).ToDictionary();
            // TokenExpressions = tokenExpr;
            // TokensByExpression = exprToken;
        }
