...
namespace Z0.Asm
{
    ...
    public class AsmOpCodeParser : Service<AsmOpCodeParser>
    {
        readonly ConstLookup<string,AsmOcToken> Expressions;

        readonly Index<AsmOcToken> _Tokens;

        readonly Index<string> HexStrings;

        public AsmOpCodeParser()
        {
            HexStrings = HexData;
            var hexcount = HexStrings.Count;
            var kinds = Symbols.index<AsmOcTokenKind>();
            var src = OpCodeTokenSet.create().View;
            var tokencount = (uint)src.Length;
            var lookup = dict<string,AsmOcToken>(hexcount + tokencount);
            for(var i=0; i<hexcount; i++)
                lookup[HexStrings[i]] = new AsmOcToken(AsmOcTokenKind.Hex8, (byte)i);

            _Tokens = alloc<AsmOcToken>(tokencount);
