...
namespace Z0
{
    ...
    public class AsmWriterChecks : Checker<AsmWriterChecks>
    {
        ...
        */
        ...
        static void row(AsmExpr asm, in AsmHexCode hex, ITextEmitter dst)
        {
        ...
        public void CheckAsmBytes(ITextEmitter log)
        {
            ...
            var block = CharBlock64.Empty;
            var buffer = block.Data;
            var length = z8;

            dst.Clear();
            and(al, bl, dst);
            length = AsmSrcEmitter.emit(inst, al, bl, ref block);
            row(text.format(slice(buffer, 0, length)), dst.Target, log);

            dst.Clear();
            and(cl, bl, dst);
            length = AsmSrcEmitter.emit(inst, cl, bl, ref block);
            row(text.format(slice(buffer, 0, length)), dst.Target, log);

            dst.Clear();
            and(dl, bl, dst);
            length = AsmSrcEmitter.emit(inst, dl, bl, ref block);
            row(text.format(slice(buffer, 0, length)), dst.Target, log);

            dst.Clear();
            and(ah, bl, dst);
            length = AsmSrcEmitter.emit(inst, ah, bl, ref block);
            row(text.format(slice(buffer, 0, length)), dst.Target, log);

            dst.Clear();
            and(ch, bl, dst);
            length = AsmSrcEmitter.emit(inst, ch, bl, ref block);
            row(text.format(slice(buffer, 0, length)), dst.Target, log);

            dst.Clear();
            and(dh, bl, dst);
            length = AsmSrcEmitter.emit(inst, dh, bl, ref block);
            row(text.format(slice(buffer, 0, length)), dst.Target, log);

            dst.Clear();
            and(bh, bl, dst);
            length = AsmSrcEmitter.emit(inst, bh, bl, ref block);
            row(text.format(slice(buffer, 0, length)), dst.Target, log);
